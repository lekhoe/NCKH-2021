...
const EvaluationBoard = () => {
  ...
  function SaveCouncil() {
    ...
    debugger
    axios.post(GET_API_STUDENTS_URL + `/HoiDongTotNghiep/InsertHoiDong/${idHocKy}/${idMonHoc}`, body, GetToken())
    setMaHoiDong('')
    setTenHoiDong('')
    setNgayBaoVe('')
  }
    ...
    <>
      <StyledSemester.Flex>
        ...
        <div className="Body">
          ...
          {showPopupAdd ? <div className="full-screen-popup">
            <div className="popup">
              <div className="popup-header">
                <span className="label-header">Hội đồng tốt nghiệp</span>
                <button className="close-popup" onClick={() => setShowPopupAdd(false)}>x</button>
              </div>
              <div className="popup-content">
                {/* <h1>Quản lý các Folder</h1> */}
                <StyledSemester.Body>
                  <div className="container-add">
                    <div className="item">
                      ...
                      <input type="text" placeholder="Mã hội đồng" defaultValue={maHoiDong} onChange={(val) => setMaHoiDong(val.target.value)} />
                    </div>
                    <div className="item">
                      <div className="item-title">Tên hội đồng</div>
                      <input type="text" placeholder="Tên hội đồng" defaultValue={tenHoiDong} onChange={(val) => setTenHoiDong(val.target.value)} />
                    </div>
                    <div className="item">
                      <div className="item-title">Ngày Bảo vệ</div>
                      <input type="date" typeof="Date" datatype onChange={(val) => setNgayBaoVe(val.target.value)} />
                    </div>
                    <div className="item-submit">
                      <button className="submit-form" onClick={() => SaveCouncil()}>Lưu</button>
                    </div>
                  </div>
                </StyledSemester.Body>
